Shader "Universal Render Pipeline/Baked Lit (Dir)"
{
    Properties
    {
        ...
        _RemapVec("Remap Vector", Vector) = (1, 1, 0, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes input)
            {
                ...
                output.uv0AndFogCoord.xy = TRANSFORM_TEX_REMAP(input.uv, _BaseMap, float2(_RemapVec.x - 1., 1./_RemapVec.y));
            ...
            half3 MainLightWithGIColor(half3 color, Varyings input, half3 normalWS){
                ...
                half GIStrength = (bakedGI.r + bakedGI.g + bakedGI.b) * 0.33;

                // From Lighting.hlsl #608 SubtractDirectMainLightFromLightmap()
                ...
                color = color * (bakedGI + lambert * GIStrength);
                // color = half3(input.lightmapUV.xy, 0);
                return color;
            }
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                color = MainLightWithGIColor(color, input, normalWS);
#if defined(_EMISSION)
                color += SampleEmission(uv, _EmissionColor.rgb, TEXTURE2D_ARGS(_EmissionMap, sampler_EmissionMap));
#endif
        ...
        Pass
        {
            ...
            #pragma fragment UniversalFragmentMetaBakedDirLit

            #include "BakedLitDirInput.hlsl"
            #include "Packages/com.unity.render-pipelines.universal/Shaders/BakedLitMetaPass.hlsl"

            half4 UniversalFragmentMetaBakedDirLit(Varyings input) : SV_Target
            {
                MetaInput metaInput = (MetaInput)0;
                metaInput.Albedo = _BaseColor.rgb * SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, input.uv).rgb;
                metaInput.Emission = _EmissionColor.rgb * SAMPLE_TEXTURE2D(_EmissionMap, sampler_EmissionMap, input.uv).rgb;
    ...
    CustomEditor "UnityEditor.Rendering.Universal.ShaderGUI.BakedLitDirShader"
}
